...
namespace SharpNeat.SpeciationStrategies
{
    ...
    public class KMeansBehaviourClusteringStrategy<TGenome> : ISpeciationStrategy<TGenome>
        where TGenome : class, IGenome<TGenome>
    {
        const int __MAX_KMEANS_LOOPS = 5;
        private readonly GenomeRepository repository;
        ...
        /// </summary>
        public KMeansBehaviourClusteringStrategy(GenomeRepository repository, IDistanceMetric distanceMetric)
        {
        ...
        public void SpeciateGenomes(IList<TGenome> genomeList, IList<Specie<TGenome>> specieList)
        {
            ...
            for(int i=0; i<specieCount; i++)
            {
                ...
                specie.Centroid =  repository.GetPosition(genomeList[i]);
            }
        ...
        private CoordinateVector CalculateSpecieCentroid(Specie<TGenome> specie)
        {
            ...
            if(1 == specie.GenomeList.Count) {
                return new CoordinateVector(repository.GetPosition(specie.GenomeList[0]).CoordArray);
            }
            ...
            for(int i=0; i<count; i++) {
                coordList.Add(repository.GetPosition(genomeList[i]));
            }
        ...
        private TGenome[] GetGenomesByDistanceFromSpecie(IList<TGenome> genomeList, IList<Specie<TGenome>> specieList)
        {
            ...
            {
                TGenome genome = genomeList[i];
                double distance = _distanceMetric.GetDistance(repository.GetPosition(genome), specieList[genome.SpecieIdx].Centroid);
        ...
        private Specie<TGenome> FindClosestSpecie(TGenome genome, IList<Specie<TGenome>> specieList)
        {
            ...
            double closestDistance = _distanceMetric.GetDistance(repository.GetPosition(genome), closestSpecie.Centroid);

            // Measure distance to all remaining species.
            int speciesCount = specieList.Count;
            for(int i=1; i<speciesCount; i++)
            {
                double distance = _distanceMetric.GetDistance(repository.GetPosition(genome), specieList[i].Centroid);
